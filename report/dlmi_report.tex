...
\begin{document}
	...
	As explained above, the standard MIL assumption implies that a patient is classified \emph{positive} as soon as the model identifies a suspicious instance in it. During training, it assumes there must be at least $1$ \emph{positive} instance for a \emph{positive} patient. During testing, if the top-$1$ instance probability is above the classification threshold $\tau_{class}$, it classifies the entire patient as positive. During testing, instead of letting the top-$1$ instance decide for the whole patient, it makes more sense to derive the patient's prediction by looking at its top-$k$ instances' probabilities.\\
	\\
